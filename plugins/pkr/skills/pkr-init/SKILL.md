...
5. **根据 source 类型写入条件字段**：
   ...
   - **source=计划** → 无需额外条件字段
6. **所有文档必须写入模块子目录**（不允许直接放在根目录下）：
   - **source=框架:xxx** → 写入子目录 `{xxx}/`（如 `docs/capabilities/springboot/cache.md`）
     - `module` 字段填入框架名：`springboot`
     - `name` 字段：`springboot/cache`
   - **source=项目自有** → 写入子目录 `{项目模块名}/`（如 `docs/capabilities/myapp/workflow-engine.md`）
     - 项目模块名从项目配置推断：Java 用 `pom.xml` 的 artifactId / `build.gradle` 根项目名，Node 用 `package.json` 的 name 去掉 scope，其他用项目根目录名
     - `module` 字段填入项目模块名
     - `name` 字段：`myapp/workflow-engine`
   - **source=计划** → 写入子目录 `{预期模块名}/`（用户指定或从描述推断）
   - 自动创建子目录（如不存在）
